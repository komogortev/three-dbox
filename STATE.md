...
- Spawn calibration **DONE 2026-06-09** — 4 confirmed ground-level points: `yard-center (9,1)`, `mainhall-interior-west (12.8,-12.3)`, `mainhall-interior-east (23.6,-12.1)`, `otherparts-ground-east (19.6,-12.6)`. Elevated west point `(-30,2)` removed (floor at Y≈5 above terrain probe start Y=1 → would fall through). `spawnFallback` updated to `(9,1)`. Entity `0345` spawn-volumes are EMPTY nodes — no marker quads, cannot locate via debugMarkers.
