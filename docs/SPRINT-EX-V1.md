...
➡️ **EX-1.5 done (2026-06-13). Next: EX-2 — navigation reliability.** Findings: `docs/EX-1-FINDINGS.md`.
EX-1.5 decimated the player mesh `dfist_base.glb` **960,108 → 38,404 tris** (gltf-transform `weld` + `simplify --ratio 0.04 --error 0.01`, re-Draco; 3.05 MB → 639 KB). Skin weights + joints preserved; 40-bone skeleton **mixer-verified live** (clean idle deformation, sane bbox, no console errors). EX-1's #1 perf lever is now resolved — the character dropped from ~73% of scene geometry to a normal game-character budget, so EX-2's frame-time numbers will finally be honest. (EX-1 recap: lag driver was the un-decimated mesh, not draw calls — 120–180 typ / 463 worst; render vertex-bound; pixel-ratio cap shipped; BatchedMesh deferred.)
