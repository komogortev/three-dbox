...
| **NEW: character mesh optimisation** | **Add a pre-EX-2 / EX-1.5 asset task: decimate `dfist_base.glb` from 960K → ~30–50K tris** (gltf-transform `weld` + `simplify`, preserving skin weights — verify rig/animation after). Highest-ROI perf win in the whole track. The untracked alt-skins `dfist.glb` / `dfist_armored.glb` (33 MB each) are even heavier and must be optimised before they're ever activated. |
